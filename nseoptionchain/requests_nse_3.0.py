...
# Keep pulling option chain till 15:40
while now_time < cutoff_datetime:
    print("inside")
    cookie_dict = {
        "_ga": "GA1.2.1735051097.1517463640",
    ...
    filename1 = "BNF_OC_" + now_time.strftime("%d%m%Y")+".txt"

    # while time.localtime
    os.chdir("./")
    try:
        option_chain_file = open(filename, "w")
        option_chain_file.write(response.content.decode())
        option_chain_file.close()
    except OSError as error:
        print(error)

    print('File created at' + now_time.strftime("%H%M%S"))
    time.sleep(200)
print("Operation complete for the day")
